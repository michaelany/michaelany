...
const RU = {
  ...
  about: {
    ...
    feature: {
      projects: {
        ...
        text: `В моем портфолио зарубежные и отечественные коммерческие проекты - от одностраничных лендингов до крупных веб-приложений. Подробнее в <0>${common.title.portfolio}</0>`,
      },
      position: {
  ...
  skills: {
    ...
    feature: {
      ...
      efficient: {
        ...
        text:
          'Для любой задачи стараюсь найти понятное и оптимальное решение. Знаю в каких случаях нужно использовать конкретный метод, паттерн, библиотеку или инструмент',
      },
      markup: {
      ...
      modules: {
        ...
        text:
          'Избегаю чрезмерной вложенности, следую принципу разделения ответственности, {{0}} и {{1}}. Каждый модуль, класс или компонент отвечает за один набор функций. Конфигурирую систему сборки проекта',
      },
      tools: {
      ...
      code: {
        ...
        text:
          'Пишу самодокументируемый код в декларативном стиле. Изучаю практики популярных стайлгайдов. Использую {{0}} и {{1}}. Читаемый код важнее быстрого',
      },
      test: {
        label: 'Рефакторю, оптимизирую и тестирую',
        text:
          'Проверяю работу проекта в различных браузерах и устройствах. Провожу рефакторинг кода. Замеряю и ускоряю загрузку ресурсов. Минифицирую и оптимизирую файлы, изображения',
      },
      tasks: {
  ...
  portfolio: {
    limit: 'Коммерческие проекты, выпущенные в продакшн и доступные к показу',
