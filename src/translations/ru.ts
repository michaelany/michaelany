...
const RU = {
  ...
  about: {
    ...
    feature: {
      ...
      perfect: {
        ...
        text: 'Качество — мой главный принцип. С вниманием отношусь к каждой детали проекта, но опыт и дедлайны помогают сохранять баланс между скоростью и результатом. Ценю консистентность и системный подход. Люблю простоту и порядок во всем',
      },
      education: {
  ...
  skills: {
    text: 'Разрабатываю <0>приложения</0>, <1>многостраничные сайты</1>, <2>лендинги</2> и <3>админки</3>. Любимый стек — <4>{{0}}</4> + <4>{{1}}</4> + <4>{{2}}</4> + <4>{{3}}</4>. Монтирую <5>{{4}}-ролики</5> в {{5}}',
    feature: {
      interface: {
