...
const RU = {
  ...
  about: {
    text: `Я <0>инженер-программист</0> с 10-летним опытом во <0>фронтенд-разработке</0>. Создаю <1>проекты</1> разного масштаба и сложности, воплощая в жизнь <0>{{0}}</0>-идеи. Веду <2>${common.title.blog}</2> на <3>{{2}}</3>. Живу в Окленде, Новая Зеландия`,
    feature: {
      projects: {
  ...
  skills: {
    text: 'Разрабатываю <0>приложения</0>, <1>многостраничные сайты</1>, <2>лендинги</2> и <3>админки</3>. Пишу на <4>{{0}}</4>. Работаю с <4>{{1}}</4> и его экосистемой, также использую <4>Vue.js</4>. Создаю <5>{{4}}-ролики</5> и монтирую их в {{5}}',
    feature: {
      interface: {
      ...
      patterns: {
        ...
        text: 'Мемоизация, виртуализация, ленивая загрузка, оптимистичные обновление, гидратация, деструктуризация, декоратор, каррирование, делегирование, рекурсия, модуль, наблюдатель, прокси, примесь, троттлинг, дебаунс..',
      },
      requests: {
        label: 'Обрабатываю серверные запросы',
        text: 'Дедупликация, перехватчик, кэширование, предзагрузка, прерывание, повтор, аутентификация, мутация, пагинация, бесконечный скролл, отслеживание прогресса, оффлайн, параллельные и зависимые запросы..',
      },
      modules: {
      ...
      test: {
        ...
        text: 'Проверяю работу проекта в различных браузерах и устройствах. Замеряю и ускоряю загрузку ресурсов. Минифицирую и оптимизирую файлы, изображения. Пишу юнит, функциональные и сквозные тесты. Провожу рефакторинг кода',
      },
      tasks: {
        label: 'Планирую и декомпозирую задачи',
        text: 'Планирую бэклог, группирую, декомпозирую и оцениваю задачи. Применяю методологии {{0}} и {{1}}. Пользуюсь таск-менеджерами и собственной системой планирования',
      },
      mentor: {
