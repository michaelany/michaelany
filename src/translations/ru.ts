...
const RU = {
  ...
  about: {
    text: `Я инженер-программист, занимаюсь <0>фронтенд разработкой</0>. Создаю <1>проекты</1> любой сложности с нуля, воплощая в жизнь {{0}} идеи. Веду собственный <2>${common.title.vlog}</2> на <3>{{2}}</3>. Живу в Окленде, Новая Зеландия`,
    feature: {
      projects: {
      ...
      vlogger: {
        label: 'Автор канала на {{0}}',
        extraLabel: 'и автор канала на {{0}}',
  ...
  skills: {
    text: 'Разрабатываю <0>приложения</0>, <1>многостраничные сайты</1>, <2>лендинги</2> и <3>админки</3>. Любимый стек - <4>{{0}}</4> + <4>{{1}}</4> + <4>{{2}}</4> + <4>{{3}}</4> + <4>{{4}}</4>. Делаю <5>{{5}}-ролики</5> в {{6}}',
    feature: {
      interface: {
  ...
  experience: {
    ...
    job: {
      ...
      achievement: {
        ...
        routing: 'Создал собственный роутинг в одном из приложений',
        animations:
  ...
  vlog: {
    description:
      '<0>Всем привет! {{0}}</0><0>Меня зовут Миша. Профессионально занимаюсь фронтенд разработкой {{1}} лет. Создаю проекты любой сложности с нуля, воплощая в жизнь UI/UX идеи. Прошел путь от верстальщика до тимлида. Работаю в новозеландской компании 7 glyphs Ltd. {{2}}</0><0>В апреле 2023 переехал из России в Новую Зеландию. Налаживаю отношения с хоббитами и альпаками {{3}}</0><0>На этом канале я рассказываю о своем пути в программировании и жизни, а также делюсь опытом и знаниями. Надеюсь, будет интересно и, может быть, даже немного полезно {{4}}</0><0>Всем мира и любви {{5}}</0>',
    sort: {
      new: 'Новые',
      old: 'Старые',
    },
