...
const RU = {
  ...
  about: {
    ...
    feature: {
      ...
      geek: {
        label: 'Техногик',
        text: 'Люблю технику, игры, аниме и {{0}}-культуру. Обожаю {{1}}, особенно серии игр {{2}} и {{3}}. В восторге от сериала «Аркейн». Интересуюсь гаджетами и железом. Собрал свой идеальный <0>сетап</0>',
      },
      resident: {
        label: 'Резидент Новой Зеландии',
        text: 'Переехал в Новую Зеландию {{0}} года назад. Получил резидентство по профессии {{1}}. Теперь живу в стране, о которой всегда мечтал',
      },
      sport: {
        label: 'Занимаюсь спортом',
        text: 'Физическая нагрузка особенно важна для разработчика, поэтому каждый день я занимаюсь спортом. Люблю калистенику, фитнес и хайкинг. А когда все задачи закрыты и в океане хороший серф-прогноз, меня можно найти на доске среди волн',
      },
      travel: {
        label: 'Люблю путешествовать',
        text: 'Посетил <0>{{0}} стран</0>. Когда-то я поднялся на вершину Фудзиямы, выпил эля в таверне «Зеленый дракон» в Хоббитоне и пережил встречу с диким медведем на перевале Дятлова',
      },
      love: {
  ...
  skills: {
    text: 'Разрабатываю <0>приложения</0>, <1>многостраничные сайты</1>, <2>лендинги</2> && <3>админки</3>. Любимый стек — <4>{{0}}</4> + <4>{{1}}</4> + <4>{{2}}</4> + <4>{{3}}</4>. Также работаю с <4>Vue.js</4>. Для SSR-проектов использую <4>Next.js</4> || <4>Nuxt</4>. Создаю <5>{{4}}-ролики</5> && монтирую их в <4>{{5}}</4>',
    feature: {
      interface: {
        label: 'Создаю удобные и надежные интерфейсы',
        text: 'Разрабатываю аккуратные интерфейсы с понятным взаимодействием и продуманной бизнес-логикой. Понимаю, когда стоит использовать конкретный элемент, компонент или виджет. Обрабатываю ошибки, Забочусь о безопасности и SEO',
      },
      performance: {
        label: 'Пишу современный и производительный код',
        text: 'Использую стандарты {{0}}, {{1}}, {{2}}, {{3}}, {{4}} и т.д. Избегаю лишних операций и утечек памяти. Мемоизирую тяжелые вычисления, оптимизирую производительность и рендеринг',
      },
      architecture: {
        label: 'Проектирую модульную и масштабируемую архитектуру',
        text: 'Продумываю архитектуру под каждый проект. Использую модульный подход, кастомный FSD и принципы SOLID. Создаю понятную структуру кода с разделением ответственности, которую легко поддерживать и развивать',
      },
      code: {
        label: 'Пишу чистый и поддерживаемый код',
        text: 'Пишу самодокументируемый код в декларативном стиле. Использую {{0}}, {{1}} и проверенные стайлгайды. Следую принципам DRY, KISS и YAGNI. Регулярно провожу рефакторинг. Читаемый код важнее быстрого',
      },
      state: {
        label: 'Управляю состоянием приложения',
        text: 'Разделяю локальное, глобальное, серверное и URL-состояние приложения. Выстраиваю предсказуемый поток данных без дублирования. Продумываю загрузку, кеширование, синхронизацию и обновление данных',
      },
      tools: {
        label: 'Использую актуальные библиотеки и инструменты',
        text: 'Слежу за развитием технологий. Люблю React-экосистему. Использую зависимости осознанно, каждая решает конкретную задачу. Подбираю фреймворки, библиотеки, утилиты и инструменты под специфику проекта. Мой <0>основной набор</0>',
      },
      patterns: {
        label: 'Применяю множество паттернов и техник',
      ...
      markup: {
        ...
        text: 'Верстаю макеты пиксель в пиксель. Использую современные возможности CSS и семантичные HTML-теги. Не болею «диватозом». Учитываю особенности разных браузеров и забочусь о доступности. Верстаю письма',
      },
      responsive: {
      ...
      animation: {
        ...
        text: 'Создаю производительные JavaScript, CSS, SVG и WebGL-анимации: плавные переходы, микроинтеракции, интерактивные сценарии, скролл-анимации и 3D-эффекты. Делаю их плавными и уместными. Люблю GSAP, Motion и Three.js',
      },
      media: {
        label: 'Работаю с медиа и дизайном',
        text: 'Работаю с растровыми и векторными изображениями, видео и документами. Использую подходящие форматы файлов и оптимизирую без потери качества. Прототипирую макеты, участвую в создании дизайна. Работаю в Sketch и Figma',
      },
      test: {
        label: 'Тестирую и оптимизирую',
        text: 'Пишу юнит, интеграционные и сквозные тесты. Предпочитаю Vitest и Playwright. Создаю и интегрирую A/B-тесты для проверки продуктовых гипотез. Оптимизирую загрузку ресурсов, Lighthouse score и Core Web Vitals',
      },
      ai: {
        label: 'Использую AI-инструменты',
        text: 'Пользуюсь AI-агентами для разработки, ревью и анализа. Нравится Claude Code и Codex. Занимаюсь промпт-инжинирингом, валидирую AI output, работаю с MCP. Понимаю возможности и ограничения генеративного AI',
      },
      integrations: {
        label: 'Интегрирую сторонние сервисы',
        text: 'Подключаю платежные системы, headless CMS, аналитику и внешние платформы. Есть опыт интеграции SDK и API Stripe, Airwallex, Strapi, Firebase, Amplitude и Google-сервисов. Адаптирую интеграции под требования проекта',
      },
      cicd: {
        label: 'Работаю с инфраструктурой и CI/CD',
        text: 'Использую CI/CD, GitHub Actions, Cloudflare, Azure, Vercel, AWS и другие облачные сервисы для деплоя. Настраиваю пайплайны сборки, автозапуск тестов и скриптов, окружения и вспомогательные процессы для стабильных релизов',
      },
      tasks: {
  ...
  experience: {
    text: `<0>${common.occupationLead}</0> в <1>{{0}}</1>. Профессионально занимаюсь фронтенд-разработкой {{1}} лет. Рассказываю о своем пути в программировании и жизни, а также делюсь опытом и знаниями в своем <2>${common.title.blog}е</2>`,
    projects: 'Проекты',
