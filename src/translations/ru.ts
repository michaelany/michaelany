...
const RU = {
  ...
  skills: {
    text: 'Разрабатываю <0>приложения</0>, <1>многостраничные сайты</1>, <2>лендинги</2> && <3>админки</3>. Любимый стек — <4>{{0}}</4> + <4>{{1}}</4> + <4>{{2}}</4> + <4>{{3}}</4>. Также работаю с <4>Vue.js</4>. Для SSR-проектов использую <4>Next.js</4> || <4>Nuxt</4>. Предпочитаю <4>{{4}}</4> && <4>{{5}}</4> в качестве AI-ассистентов. Создаю <5>{{6}}-ролики</5> && монтирую их в <4>{{7}}</4>',
    feature: {
      interface: {
